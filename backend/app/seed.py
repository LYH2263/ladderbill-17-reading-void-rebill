...
def _migrate(conn):
    """为旧库补齐作废/重测相关列（SQLite ADD COLUMN 守卫式迁移）。"""
...
def init_db():
    ...
    if conn.execute("SELECT COUNT(*) c FROM accounts").fetchone()["c"] == 0:
        ...
        conn.execute(
            ...
            ("bill", 1, 1, json.dumps({"kwh": 120, "peak": False}), json.dumps(bill1, ensure_ascii=False)),
        )
        cmp2 = compare_plain_vs_peak(400, tiers, 1.2)
        conn.execute(
            "INSERT INTO calc_runs(kind, account_id, reading_id, input_json, result_json, created_at) VALUES (?,?,?,?,?,datetime('now'))",
            ("compare", 2, 2, json.dumps({"kwh": 400}), json.dumps(cmp2, ensure_ascii=False)),
        )
        conn.commit()
    conn.close()
